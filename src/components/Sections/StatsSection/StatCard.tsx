...
const StatCard = memo<StatCardProps>(({ stat }) => (
  <div 
    ...
    role="button"
    tabIndex={0}
    aria-label={`סטטיסטיקה: ${stat.number}${stat.suffix} ${stat.label}`}
  >
    <div className="relative flex items-center justify-center p-4 md:p-8 h-full z-10">
      <div className="relative z-10 flex flex-col items-center justify-center space-y-2 md:space-y-4">
        ...
        <p className="text-white/70 text-center text-xs md:text-sm font-medium group-hover:text-white/90 group-active:text-white group-focus:text-white transition-colors duration-700 md:duration-300" dir="rtl">
          {stat.label}
        </p>
      </div>
    </div>
